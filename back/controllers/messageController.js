  ...
  try {
    ...
    await db.promise().query(
      `UPDATE messages SET is_read = 1
       WHERE id = ? AND to_user = ? AND deleted_by_receiver = 0`,
      [messageId, userId]
    );
...
exports.getConversation = async (req, res) => {
  try {
    ...
    await db.promise().query(
      `UPDATE messages SET is_read = 1
       WHERE from_user = ? AND to_user = ? AND deleted_by_receiver = 0`,
      [otherUserId, userId]
    );
...
exports.getChatMessages = async (req, res) => {
  try {
    ...
    await db.promise().query(
      `UPDATE messages SET is_read = 1
       WHERE chat_id = ? AND to_user = ? AND is_read = 0 AND deleted_by_receiver = 0`,
      [chatId, userId]
    );
