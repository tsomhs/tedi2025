...
exports.createAuction = (req, res) => {
  ...
  const { itemName, categories, firstBid, buyPrice, location, country, started, ends, description } = req.body;
...
exports.getAuctionById = (req, res) => {
  ...
  db.query(query, [auctionId], (err, results) => {
    ...
    db.query(
      ...
      [auctionId],
      (err, categories) => {
        if (err) return res.status(500).json({ msg: 'Error loading categories', err });
...
exports.updateAuction = (req, res) => {
  ...
  db.query(checkQuery, [itemId], (err, results) => {
    ...
    if (started <= now) return res.status(400).json({ msg: 'Auction already started' });
    ...
    db.query(
      ...
      (err, bidResults) => {
        ...
        if (bidResults[0].bidCount > 0) {
          return res.status(400).json({ msg: 'Cannot update. Auction already has bids.' });
        }

        const updateQuery = `
          UPDATE items SET name = ?, description = ?, ends = ?, buy_price = ? WHERE id = ?`;
        db.query(updateQuery, [itemName, description, ends, buyPrice, itemId], err => {
...
exports.deleteAuction = (req, res) => {
  ...
  db.query('SELECT * FROM items WHERE id = ?', [itemId], (err, results) => {
    ...
    db.query('SELECT COUNT(*) AS bidCount FROM bids WHERE item_id = ?', [itemId], (err, bidResults) => {
      if (err) return res.status(500).json({ msg: 'Error checking bids', err });
      if (bidResults[0].bidCount > 0) {
        return res.status(400).json({ msg: 'Cannot delete. Auction already has bids.' });
      }
...
exports.searchAuctions = (req, res) => {
  const { category, q, minPrice, maxPrice, location, page = 1, limit = 10 } = req.query;
  ...
  if (maxPrice) {
    baseQuery += ' AND (SELECT MAX(b.amount) FROM bids b WHERE b.item_id = i.id) <= ?';
