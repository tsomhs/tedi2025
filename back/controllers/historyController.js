...
exports.logView = (req, res) => {
  ...
  const insertQuery = `
    INSERT INTO view_history (user_id, item_id)
    VALUES (?, ?)
  `;
...
exports.getRecommendations = (req, res) => {
  ...
  db.query(categoryQuery, [userId], (err, catResults) => {
    if (err) return res.status(500).json({ msg: 'Error fetching categories', err });
    ...
    const recQuery = `
      ...
      WHERE ic.category_name IN (?) 
        AND i.id NOT IN (
          SELECT item_id FROM view_history WHERE user_id = ?
        )
      LIMIT 10
    `;

    db.query(recQuery, [categories, userId], (err, recResults) => {
      if (err) return res.status(500).json({ msg: 'Error fetching recommendations', err });
