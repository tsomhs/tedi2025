...
function buildInteractions(raw, wBids = 1.0, wViews = 0.3) {
  ...
  for (const r of bids) {
    const u = String(r.user_id), i = String(r.item_id);
...
function trainMF(interactions, nUsers, nItems, { factors = 32, lr = 0.02, reg = 0.02, epochs = 30 }) {
...
async function main() {
  ...
  if (data.length === 0) {
    console.error('No interactions found (bids/view_history). Run parseEbayData first.');
    process.exit(1);
  }
  console.log(`[MF] Users: ${users.length} | Items: ${items.length} | Interactions: ${data.length}`);

  const { P, Q } = trainMF(data, users.length, items.length, { factors, lr, reg, epochs });
  ...
  fs.writeFileSync('models/mf_meta.json', JSON.stringify({
